...
void Basis60(input_stream<float> *  __restrict datain, output_stream<float>*  __restrict dataout)
{
    ...
    for(int j=0;j<x_dim;j++)chess_prepare_for_pipelining{
        ...
        N0 =(grid[0] <= temp1 && temp1 < grid[0 + 1]) ? 1.0 : 0.0;

        temp2 = ((temp1 - grid[0]) * (temp1 - grid[0]) * (temp1 - grid[0])) / 
               ((grid[3] - grid[0]) * (grid[2] - grid[0]) * (grid[1] - grid[0])); //can be further simplified by replacing grid with fixed value
        temp3 = temp2*N0;
