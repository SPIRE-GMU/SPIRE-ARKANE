...
void vertical_6(input_stream<float> *  __restrict datain, input_stream<float> *  __restrict result_in,output_stream<float>*  __restrict dataout)
{
    ...
    //transit to vertical direction. // very important to shift the grid
    ...
    for(int j=0;j<x_dim;j++)chess_prepare_for_pipelining{
        for(int z=0;z<6;z++){
            temp1 = readincr(result_in);
            writeincr(dataout,temp1);
        }
        temp0=readincr(datain);
        
        N0 =(grid[3] <= temp0 && temp0 < grid[3 + 1]) ? 1.0 : 0.0;
        temp1=readincr(datain);

        temp2 = ((grid[4] - temp0) * (grid[4] - temp0) * (grid[4] - temp0)) / 
               ((grid[4] - grid[1]) * (grid[4] - grid[2]) * (grid[4] - grid[3])); //can be further simplified by replacing grid with fixed value
