...
//                ((grid[3] - grid[0]) * (grid[2] - grid[1]) * (grid[2] - grid[0])) + 
//                  ((x-grid[0])*(grid[3]-x)*(x-grid[1]))/((grid[3]-grid[0])*(grid[3]-grid[1])*(grid[2]-grid[1])) +
//                  ((x-grid[1])*(grid[4]-x)*(x-grid[1]))/((grid[4]-grid[0])*(grid[3]-grid[1])*(grid[2]-grid[1]));
...
void horizontal_1(input_stream<float> *  __restrict datain, output_stream<float>*  __restrict dataout)
{
    ...
    for(int j=0;j<x_dim;j++)chess_prepare_for_pipelining{
        ...
        N0 =(grid[1] <= temp0 && temp0 < grid[1 + 1]) ? 1.0 : 0.0;

        temp1=readincr(datain);

        temp2 = ((temp0 - grid[0]) * (temp0 - grid[0]) * (grid[2] - temp0)) / 
               ((grid[3] - grid[0]) * (grid[2] - grid[1]) * (grid[2] - grid[0]));
